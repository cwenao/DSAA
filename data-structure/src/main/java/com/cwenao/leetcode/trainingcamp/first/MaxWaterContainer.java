/*
 * Company
 * Copyright (C) 2014-2020 All Rights Reserved.
 */
package com.cwenao.leetcode.trainingcamp.first;

/**
 * 11. 给你 n 个非负整数 a1，a2，...，an，每个数代表坐标中的一个点(i,ai) 。
 * 在坐标内画 n 条垂直线，垂直线 i 的两个端点分别为(i,ai) 和 (i, 0) 。
 * 找出其中的两条线，使得它们与 x 轴共同构成的容器可以容纳最多的水。
 *
 * 说明：你不能倾斜容器。
 *
 * 来源：力扣（LeetCode）
 * 链接：https://leetcode-cn.com/problems/container-with-most-water
 * @author cwenao
 * @version $Id MaxWaterContainer.java, v1.0.0 2020-11-23 10:26 cwenao Exp $$
 */
public class MaxWaterContainer {

    /**
     * http://image.cwenao.jikedo.com/20201123102854.png
     * 输入：[1,8,6,2,5,4,8,3,7] 输出：49 解释：图中垂直线代表输入数组
     * [1,8,6,2,5,4,8,3,7]。在此情况下，容器能够容纳水（表示为蓝色部分）的最大值为 49。
     */

    public int maxArea(int[] height) {
        return maxAreaByDoublePoint(height);
    }

    private int maxAreaByDoublePoint(int[] height) {

        int result = 0;

        if (height == null || height.length == 0) {
            return result;
        }
        int left = 0, right = height.length - 1;
        while (left < right) {
            int currentArea = Math.min(height[left], height[right]) * (right - left);
            result = Math.max(currentArea, result);

            if (height[left] <= height[right]) {
                ++left;
            } else {
                --right;
            }
        }
        return result;
    }

    public static void main(String[] args) {
        int[] height = new int[]{1, 8, 6, 2, 5, 4, 8, 3, 7};
        MaxWaterContainer maxWaterContainer = new MaxWaterContainer();
        System.out.println(maxWaterContainer.maxArea(height));
    }

}
